...
if (!class_exists('EVDPL_Customizer')):

    class EVDPL_Customizer {
        ...
        public function includes() {
            require_once $this->_plugin_class->include_path('class-evdpl-customize-heading.php');
            require_once $this->_plugin_class->include_path('class-evdpl-customize-alpha-color-control.php');
        }
        ...
        public function add_panel($wp_customize) {
            $wp_customize->add_panel($this->_settings_name, array(
                ...
                'title' => esc_html__('Product Variation Swatches for WC', 'woocommerce-variation-swatches'),
            ));
        }
        ...
        public function add_section($wp_customize) {
            ...
            foreach ($this->_fields as $panel) {
                $section_id = sprintf('%s_%s', $this->_settings_name, esc_attr($panel['id']));

                $wp_customize->add_section($section_id, array(
                    'title' => esc_attr($panel['title']),
                    'panel' => esc_attr($this->_settings_name),
                ));
                ...
                foreach ($panel['sections'] as $section) {
                    ...
                    if (!isset($section['customize_hidden']) || !$section['customize_hidden']) {
                        new EVDPL_Customize_Heading($wp_customize,esc_attr($section_id),esc_attr($section['title']));
                    }
                    ...
                    foreach ($section['fields'] as $field) {
                        ...
                        $setting_id = sprintf('%s[%s]', esc_attr($this->_settings_name),esc_attr($field['id']));

                        $default_value = esc_attr($field['default']);

                        // Theme Support
                        if ($theme_support) {
                            $default_value = isset($theme_support[0][$field['id']]) ? esc_attr($theme_support[0][$field['id']]) : esc_attr($field['default']);
                        }
                        ...
                        $control_options = array(
                            'label' => esc_attr($field['title']),
                            'description' => esc_attr($field['desc']),
