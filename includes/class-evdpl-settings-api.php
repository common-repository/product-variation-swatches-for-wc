...
if (!class_exists('EVDPL_Settings_API')):

    class EVDPL_Settings_API {

        private $setting_name = 'woocommerce_variation_swatches';
        private $setting_reset_name = 'reset';
        private $show_pro_name = 'pro';
        private $transient_setting_name = '_temp_woocommerce_variation_swatches_options';
        private $cache_key = 'woocommerce_variation_swatches_options';
        private $theme_feature_name = 'woocommerce-variation-swatches';
        private $slug;
        ...
        public function __construct() {
            ...
            $this->plugin_class = woocommerce_variation_swatches();

            $this->settings_name = apply_filters('evdpl_settings_name', $this->setting_name);
            $this->setting_reset_name = apply_filters('evdpl_reset_settings_name', $this->setting_reset_name);

            $this->slug = sprintf('%s-settings', sanitize_key($this->plugin_class->dirname()));
            // license_key
            $this->fields = apply_filters('evdpl_settings', $this->fields);
            $this->reserved_key = sprintf('%s_reserved', esc_attr($this->settings_name));
            $this->reserved_fields = apply_filters('evdpl_reserved_fields', array());
            ...
            if (apply_filters('show_evdpl_settings_link_on_admin_bar', false)):
                add_action('wp_before_admin_bar_render', array($this, 'add_admin_bar'), 999);
            endif;

            add_action('admin_footer', array($this, 'admin_inline_js'));

            if (apply_filters('show_evdpl_settings_on_customizer', false)):
                new EVDPL_Customizer($this->theme_feature_name, $this->plugin_class, esc_attr($this->settings_name), $this->fields);
            endif;

            do_action('evdpl_setting_api_init', $this);
        }
        ...
        public function get_reserved($key = false) {
            ...
            if ($key) {
                return isset($data[$key]) ? esc_attr($data[$key]) : null;
            } else {
                return $data;
            }
        }
        ...
        public function save_reserved($value) {
            ...
            foreach ((array) $this->reserved_fields as $fieldKey) {
                if (!empty($value[$fieldKey])) {
                    $reserved_data[$fieldKey] = esc_attr($value[$fieldKey]);
                }
            }
        ...
        public function before_update($value, $old_value, $option) {
            //if ( esc_attr($this->settings_name) === $option ) {
            // Here We will do magic :D
            // delete_transient( $this->transient_setting_name );
            //}

            $this->save_reserved($value);
            do_action(sprintf('before_update_%s_settings', esc_attr($this->settings_name)), $this);
        ...
        public function admin_inline_js() {
            ...
            <script type="text/javascript">
                jQuery(function ($) {
                    $('#<?php echo esc_attr($this->slug) ?>-wrap').on('click', '.nav-tab', function (event) {
        ...
        public function add_menu() {
            ...
            $page_title = esc_html__('Product Variation Swatches for WC Settings', 'woocommerce-variation-swatches');
            $menu_title = esc_html__('Swatches', 'woocommerce-variation-swatches');
            add_menu_page($page_title, $menu_title, 'edit_theme_options', esc_attr($this->slug), array(
                $this,
                'settings_form'
        ...
        public function add_admin_bar() {
            ...
            $url = admin_url(sprintf('admin.php?page=%s', esc_attr($this->slug)));
            $menu_title = esc_html__('Swatches Settings', 'woocommerce-variation-swatches');

            $args = array(
                'id' => esc_attr($this->settings_name),
                'title' => $menu_title,
                'href' => esc_url($url),
                'meta' => array(
                    'class' => sprintf('%s-admin-toolbar', esc_attr($this->slug))
                )
            );
            ...
            if (!is_admin() && class_exists('WooCommerce') && ( is_singular('product') || is_shop() )) {
                $wp_admin_bar->add_menu(array(
                    'id' => 'evdpl-clear-transient',
                    'title' => esc_html__('Clear transient', 'woocommerce-variation-swatches'),
                    'href' => esc_url(remove_query_arg(array(
                        'variation_id',
                        'remove_item',
                        'add-to-cart',
                        'added-to-cart'
                                    ), add_query_arg('evdpl_clear_transient', ''))),
                    'parent' => esc_attr($this->settings_name),
                    'meta' => array(
                        'class' => sprintf('%s-admin-toolbar-cache', esc_attr($this->slug))
                    )
                ));
            }

            do_action('evdpl_admin_bar_menu', $wp_admin_bar, esc_attr($this->settings_name));
        }
        ...
        public function plugin_action_links($links) {
            ...
            $url = admin_url(sprintf('admin.php?page=%s', esc_attr($this->slug)));
            $plugin_links = array(sprintf('<a href="%s">%s</a>', esc_url($url), esc_html__('Settings', 'woocommerce-variation-swatches')));

            return array_merge($plugin_links, $links);
        }

        private function set_default($key, $type, $value) {
            $this->defaults[$key] = array('id' => esc_attr($key), 'type' => esc_attr($type), 'value' => esc_attr($value));
        }

        private function get_default($key) {
            return isset($this->defaults[$key]) ? esc_attr($this->defaults[$key]) : null;
        }
        ...
        public function set_defaults() {
            foreach ($this->fields as $tab_key => $tab) {
                $tab = apply_filters('evdpl_settings_tab', $tab);

                foreach ($tab['sections'] as $section_key => $section) {

                    $section = apply_filters('evdpl_settings_section', $section, $tab);

                    $section['id'] = !isset($section['id']) ? esc_attr($tab['id'] . '-section') : esc_attr($section['id']);

                    $section['fields'] = apply_filters('evdpl_settings_fields', $section['fields'], $section, $tab);

                    foreach ($section['fields'] as $field) {
                        if (isset($field['pro'])) {
                            continue;
                        }
                        $field['default'] = isset($field['default']) ? esc_attr($field['default']) : null;
                        $this->set_default(esc_attr($field['id']), esc_attr($field['type']), esc_attr($field['default']));
                    }
                }
            }
        }
        ...
        public function delete_settings() {
            ...
            do_action(sprintf('delete_%s_settings', esc_attr($this->settings_name)), $this);
        ...
        public function get_option($option) {
            ...
            if (current_theme_supports($this->theme_feature_name)) {
                ...
                $default['value'] = isset($theme_support[0][$option]) ? esc_attr($theme_support[0][$option]) : esc_attr($default['value']);
            }

            $default_value = isset($default['value']) ? esc_attr($default['value']) : null;
        ...
        public function is_reset_all() {
            return isset($_GET['page']) && ( sanitize_text_field($_GET['page']) == esc_attr($this->slug) ) && isset($_GET[$this->setting_reset_name]);
        }

        public function is_show_pro() {
            return isset($_GET['page']) && ( sanitize_text_field($_GET['page']) == esc_attr($this->slug) ) && isset($_GET[$this->show_pro_name]);
        }
        ...
        public function settings_init() {
            ...
            register_setting(esc_attr($this->settings_name), esc_attr($this->settings_name), array($this, 'sanitize_callback'));
            ...
            foreach ($this->fields as $tab_key => $tab) {
                ...
                foreach ($tab['sections'] as $section_key => $section) {
                    ...
                    $section['id'] = !isset($section['id']) ? esc_attr($tab['id'] . '-section-' . $section_key) : esc_attr($section['id']);

                    // Adding Settings section id
                    $this->fields[$tab_key]['sections'][$section_key]['id'] = esc_attr($section['id']);

                    add_settings_section($tab['id'] . $section['id'], $section['title'], function () use ($section) {
                        if (isset($section['desc']) && !empty($section['desc'])) {
                            echo wp_kses_post('<div class="inside">' . $section['desc'] . '</div>');
                        }
                    }, $tab['id'] . $section['id']);
                    ...
                    foreach ($section['fields'] as $field) {
                        ...
                        //$field[ 'label_for' ] = esc_attr($this->settings_name) . '[' . $field[ 'id' ] . ']';
                        ...
                        add_settings_field(esc_attr($this->settings_name) . '[' . $field['id'] . ']', $field['title'], array(
                            $this,
                            'field_callback'
        ...
        public function field_callback($field) {
            ...
            do_action('evdpl_settings_field_callback', $field);
        }
        ...
        public function radio_field_callback($args) {
            // $size    = isset( $args[ 'size' ] ) && ! is_null( $args[ 'size' ] ) ? $args[ 'size' ] : 'regular';
            $options = apply_filters("evdpl_settings_{$args['id']}_radio_options", $args['options']);
            ...
            $html .= implode('<br />', array_map(function ($key, $option) use ($attrs, $args, $value) {
                        return sprintf('<label><input %1$s type="radio"  name="%4$s[%2$s]" value="%3$s" %5$s/> %6$s</label>', $attrs, esc_attr($args['id']), esc_attr($key), esc_attr($this->settings_name), checked($value, $key, false), esc_attr($option));
        ...
        public function select_field_callback($args) {
            $options = apply_filters("evdpl_settings_{$args['id']}_select_options", $args['options']);
            $value = esc_attr($this->get_option($args['id']));
            ...
            $html = sprintf('<select %5$s class="%1$s-text" id="%2$s-field" name="%4$s[%2$s]">%3$s</select>', esc_attr($size), esc_attr($args['id']), implode('', $options), esc_attr($this->settings_name), esc_attr($attrs));
            $html .= $this->get_field_description($args);
        ...
        public function post_select_field_callback($args) {
            ...
            $options = apply_filters("evdpl_settings_{$args['id']}_post_select_options", $args['options']);
        ...
        public function text_field_callback($args) {
            ...
            $html = sprintf('<input %5$s type="text" class="%1$s-text" id="%2$s-field" name="%4$s[%2$s]" value="%3$s"/>', esc_attr($size), esc_attr($args['id']), esc_attr($value), esc_attr($this->settings_name), esc_attr($attrs));
            $html .= $this->get_field_description($args);
        ...
        public function pro_field_callback($args) {
            ...
            if ($is_html) {
                ...
                $width = isset($args['width']) ? esc_attr($args['width']) : '70%';
        ...
        public function color_field_callback($args) {
            ...
            $html = sprintf('<input type="text" %1$s class="evdpl-color-picker" id="%2$s-field" name="%4$s[%2$s]" value="%3$s"  data-default-color="%3$s" />', esc_attr($alpha), esc_attr($args['id']), esc_attr($value), esc_attr($this->settings_name));
            $html .= $this->get_field_description($args);
        ...
        public function number_field_callback($args) {
            ...
            $html = sprintf('<input %9$s type="number" class="%1$s-text" id="%2$s-field" name="%4$s[%2$s]" value="%3$s" %5$s %6$s %7$s /> %8$s', esc_attr($size), $args['id'], esc_attr($value), esc_attr($this->settings_name), esc_attr($min), esc_attr($max), $step, $suffix, $attrs);
            $html .= $this->get_field_description($args);
        ...
        public function settings_form() {
            ...
            <div id="<?php echo esc_attr($this->slug) ?>-wrap" class="wrap settings-wrap">
                ...
                <form method="post" action="<?php echo esc_url(admin_url('options.php')) ?>" enctype="multipart/form-data">
                    <?php
                    settings_errors();
                    settings_fields(esc_attr($this->settings_name));
                    ?>
                    ...
                    <div id="settings-tabs">
                        ...
                        foreach ($this->fields as $tab):
                            ...
                            $is_active = ( $this->get_last_active_tab() == esc_attr($tab['id']) );
                            ?>
                            <div id="<?php echo esc_attr($tab['id']); ?>"
                                 class="settings-tab evdpl-setting-tab"
                                 style="<?php echo!$is_active ? 'display: none' : '' ?>">
                                     <?php
                                     foreach ($tab['sections'] as $section):
                                         $this->do_settings_sections(esc_attr($tab['id'] . $section['id']));
                                     endforeach;
                                     ?>
                            </div>
                        <?php endforeach; ?>
                    ...
                    // submit_button();
                    ?>
                    <p class="submit evdpl-button-wrapper">
                        <input type="submit" id="submit" class="button button-primary" value="<?php esc_html_e('Save Changes', 'woocommerce-variation-swatches') ?>">
                        <a onclick="return confirm('<?php esc_attr_e('Are you sure to reset current settings?', 'woocommerce-variation-swatches') ?>')" class="reset" href="<?php echo esc_url($this->reset_url()) ?>"><?php esc_html_e('Reset all', 'woocommerce-variation-swatches') ?></a>
                    </p>
                </form>
            </div>
            <?php
        ...
        public function reset_url() {
            return add_query_arg(array('page' => esc_attr($this->slug), 'reset' => ''), admin_url('admin.php'));
        }

        public function settings_url() {
            return add_query_arg(array('page' => esc_attr($this->slug)), admin_url('admin.php'));
        }

        private function last_tab_input() {
            printf('<input type="hidden" id="_last_active_tab" name="%s[_last_active_tab]" value="%s">', esc_attr($this->settings_name), $this->get_last_active_tab());
        }
        ...
        public function options_tabs() {
            ...
            <h2 class="nav-tab-wrapper wp-clearfix">
                <?php foreach ($this->fields as $tabs): ?>
                    <a data-target="<?php echo esc_attr($tabs['id']) ?>" <?php echo $this->get_options_tab_pro_attr($tabs) ?> class="evdpl-setting-nav-tab nav-tab <?php echo $this->get_options_tab_css_classes($tabs) ?> " href="#<?php echo esc_attr($tabs['id']) ?>"><?php echo esc_attr($tabs['title']) ?></a>
                <?php endforeach; ?>
            </h2>
            <?php
        }
        ...
        private function get_options_tab_pro_attr($tabs) {
            // $attrs[] = ( isset( $tabs[ 'is_pro' ] ) && $tabs[ 'is_pro' ] ) ? sprintf( 'data-pro-text="%s"', apply_filters( 'evdpl_settings_tab_pro_text', 'Pro' ) ) : false;
            $attrs[] = ( isset($tabs['is_new']) && $tabs['is_new'] ) ? sprintf('data-new-text="%s"', apply_filters('evdpl_settings_tab_new_text', 'New')) : false;
        ...
        private function get_last_active_tab() {
            ...
            if (isset($_GET['tab']) && !empty($_GET['tab'])) {
                $last_tab = trim(sanitize_text_field($_GET['tab']));
            }
        ...
        private function build_dependency($require_array) {
            ...
            foreach ($require_array as $k => $v) {
                $b_array['#' . esc_attr($k) . '-field'] = esc_attr($v);
            }
        ...
        private function do_settings_fields($page, $section) {
            ...
            foreach ((array) $wp_settings_fields[$page][$section] as $field) {
                ...
                $new_html = $is_new ? wp_kses_post('<span class="evdpl-new-feature-tick">' . esc_html__('NEW', 'woocommerce-variation-swatches') . '</span>') : '';
                ...
                if (isset($field['args']['pro'])) {
                    ...
                    echo '<th scope="row" class="evdpl-settings-label">';
                    if (!empty($field['args']['label_for'])) {
                        echo '<label for="' . esc_attr($field['args']['label_for']) . '">' . esc_attr($field['title']) . wp_kses_post($new_html) . '</label>';
                    } else {
                        echo esc_attr($field['title']) . wp_kses_post($new_html);
                    }


                    echo '</th>';

                    echo '<td class="evdpl-settings-field-content">';
        ...
        public function show_pro_label_tag_content() {
            if ($this->is_show_pro()) {
                return '<span class="evdpl-show-pro-contents">Upgrade to premium &gt;&gt;</span>';
            }
        }
