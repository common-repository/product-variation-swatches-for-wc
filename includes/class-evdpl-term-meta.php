...
if ( ! class_exists( 'EVDPL_Term_Meta' ) ):
	class EVDPL_Term_Meta {
		...
		public function taxonomy_columns( $columns ) {
			...
			$new_columns['evdpl-meta-preview'] = '';
		...
		public function taxonomy_column( $columns, $column, $term_id ) {
			...
			$attribute       = evdpl_get_wc_attribute_taxonomy( $this->taxonomy );
			$fields          = evdpl_taxonomy_meta_fields( $attribute->attribute_type );
			$available_types = evdpl_available_attributes_types( $attribute->attribute_type );

			do_action( 'evdpl_global_attribute_column', $column, $term_id, $this->taxonomy, $attribute, $fields, $available_types );
		...
		public function save( $term_id, $tt_id = '', $taxonomy = '' ) {
			...
			if ( $taxonomy == $this->taxonomy ) {
						...
						if ( $field['id'] == $post_key ) {
							switch ( $field['type'] ) {
								...
								case 'checkbox':
									...
									break;
								default:
									do_action( 'evdpl_save_term_meta', esc_attr($term_id), $field, $post_value, $taxonomy );
									break;
							}
				...
				do_action( 'evdpl_after_term_meta_saved', $term_id, $taxonomy );
			}
		}
		...
		public static function generate_form_fields( $fields, $term ) {
			...
			$fields = apply_filters( 'evdpl_term_meta_fields', $fields, $term );
			...
			foreach ( $fields as $field ) {
				...
				$field = apply_filters( 'evdpl_term_meta_field', $field, $term );
				...
				$field['size']        = isset( $field['size'] ) ? esc_attr($field['size']) : '40';
				...
				$field['desc']        = ( isset( $field['desc'] ) ) ? esc_attr($field['desc']) : '';
				...
				switch ( $field['type'] ) {
					...
					case 'color':
						ob_start();
						?>
						<input name="<?php echo esc_attr($field['id']) ?>" id="<?php echo esc_attr($field['id']) ?>" type="text" class="evdpl-color-picker" value="<?php echo esc_attr($field['value']) ?>" data-default-color="<?php echo esc_attr($field['value']) ?>" size="<?php echo esc_attr($field['size']) ?>" <?php echo esc_attr($field['required'] . $field['placeholder']) ?>>
						<?php
						echo ob_get_clean();
						break;
					case 'textarea':
						ob_start();
						?>
						<textarea name="<?php echo esc_attr($field['id']) ?>" id="<?php echo esc_attr($field['id']) ?>" rows="5" cols="<?php echo esc_attr($field['size']) ?>" <?php echo esc_attr($field['required'] . $field['placeholder']) ?>><?php echo esc_attr($field['value']) ?></textarea>
						<?php
						echo ob_get_clean();
						break;
					case 'editor':
					...
					case 'select2':
						...
						$css_class         = ( $field['type'] == 'select2' ) ? 'evdpl-selectwoo' : '';
						...
						<select name="<?php echo esc_attr($field['id']) ?>" id="<?php echo esc_attr($field['id']) ?>" class="<?php echo esc_attr($css_class) ?>" <?php echo esc_attr($field['multiple']) ?>>
							...
							foreach ( $field['options'] as $key => $option ) {
								echo '<option' . selected( $field['value'], $key, false ) . ' value="' . esc_attr($key) . '">' . esc_attr($option) . '</option>';
							}
							?>
						</select>
						<?php
					...
					case 'image':
						...
						<div class="meta-image-field-wrapper">
							...
							<div class="button-wrapper">
								...
								<button type="button" class="evdpl_upload_image_button button button-primary button-small"><?php esc_html_e( 'Upload / Add image', 'woocommerce-variation-swatches' ); ?></button>
								<button type="button" style="<?php echo( empty( $field['value'] ) ? 'display:none' : '' ) ?>" class="evdpl_remove_image_button button button-danger button-small"><?php esc_html_e( 'Remove image', 'woocommerce-variation-swatches' ); ?></button>
							</div>
						</div>
						<?php
						echo ob_get_clean();
						...
						<label for="<?php echo esc_attr( $field['id'] ) ?>">
							...
							<input name="<?php echo esc_attr($field['id']) ?>" id="<?php echo esc_attr($field['id']) ?>"
								<?php checked( $field['value'], 'yes' ) ?>
								   type="<?php echo esc_attr($field['type']) ?>"
								   value="yes" <?php echo esc_attr($field['required'] . $field['placeholder']) ?>>

							<?php echo esc_attr($field['label']) ?></label>
						<?php
						echo ob_get_clean();
						break;
					default:
						do_action( 'evdpl_term_meta_field', $field, $term );
						break;
		...
		private static function placeholder_img_src() {
			return woocommerce_variation_swatches()->images_uri( 'placeholder.png' );
		}
		...
		private static function field_end( $field, $term ) {
			...
			if ( ! $term ) {
				...
				?>
				<p class="description"><?php echo esc_attr($field['desc']) ?></p></td>
				</tr>
				<?php
			}
			echo ob_get_clean();
